...
public interface ShopProductRepository extends JpaRepository<ShopProduct, Long> {
    ...
    @Query("SELECT new vn.aptech.petspa.dto.ShopProductDTO(p) FROM ShopProduct p WHERE p.name LIKE CONCAT('%', :name, '%') AND p.deleted = false")
    Page<ShopProductDTO> findByName(@Param("name") String name, Pageable pageable);

    @Query("SELECT new vn.aptech.petspa.dto.ShopProductDTO(p) FROM ShopProduct p WHERE p.category.id = :categoryId AND p.deleted = false")
    Page<ShopProductDTO> findByCategoryId(Long categoryId, Pageable pageable);

    @Query("SELECT new vn.aptech.petspa.dto.ShopProductDTO(p) FROM ShopProduct p WHERE p.name LIKE CONCAT('%', :name, '%') AND p.category.id = :categoryId AND p.deleted = false")
