...
public interface ShopProductRepository extends JpaRepository<ShopProduct, Long> {
    ...
    @Query("SELECT new vn.aptech.petspa.dto.ShopProductDTO(p.id, p.name, p.price, p.description, p.category.id, p.imageUrl) FROM ShopProduct p WHERE p.deleted = false")
    ...
    @Query("SELECT new vn.aptech.petspa.dto.ShopProductDTO(p.id, p.name, p.price, p.description, p.category.id, p.imageUrl) FROM ShopProduct p WHERE p.deleted = true")
    Page<ShopProductDTO> findAllDeleted(Pageable pageable);

    @Query("SELECT new vn.aptech.petspa.dto.ShopProductDTO(p.id, p.name, p.price, p.description, p.category.id, p.imageUrl) FROM ShopProduct p")
